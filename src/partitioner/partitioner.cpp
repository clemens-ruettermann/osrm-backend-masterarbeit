...
int Partitioner::Run(const PartitionerConfig &config)
{
    ...
    }
    {
        ...
        util::inplacePermutation(node_consumptions.begin(), node_consumptions.end(), permutation);
	    extractor::files::writeEdgeBasedNodeWeightsDurationsDistancesConsumptions(
			    config.GetPath(".osrm.enw"), node_weights, node_durations, node_distances, node_consumptions);
    }
    {
